...
//  gl_frontEnd.cpp
//
//  Created by Jean-Yves Hervé on 2018-12-05.  Updated 2022-12-04
...
extern std::mutex globalTravelerLock;
...
extern bool displayGridTraveler;
...
void drawGrid(void)
{
	//	Same as in drawTraveler above.  I wish these could be static,
	//	but it won't work for Ubuntu.
	const GLfloat DH = (GRID_PANE_WIDTH - 2.f) / numCols,
				  DV = (GRID_PANE_HEIGHT - 2.f) / numRows;
	...
	for (unsigned int i = 0; i < numRows; i++)
	{
		for (unsigned int j = 0; j < numCols; j++)
		{
			...
			if (displayGridTraveler && grid[i][j] == SquareType::TRAVELER)
			{
				//         red  green blue
