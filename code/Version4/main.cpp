...
void drawTravelers(void)
{
	...
	{
		// acquire the lock for this traveler
...
void updateMessages(void)
{
	...
	sprintf(message[2], ", to slow down travelers");
	sprintf(message[3], ". to speed up travelers");
	sprintf(message[4], "Simulation run time: %ld s", time(NULL) - launchTime);
...
void handleKeyboardEvent(unsigned char c, int x, int y)
{
	...
	case '.':
		...
		break;
	case 't':
		displayGridTraveler = !displayGridTraveler;
		break;
	default:
		ok = 1;
		break;
...
void initializeApplication(void)
{
	...
	// Allocate the array of locks, one for each traveler
	travelerLocks = new mutex[numTravelers];
	for (unsigned int i = 0; i < numTravelers; i++)
	{
		// unlocked by default
		travelerLocks[i].unlock();
	}
...
void travelerUpdate(vector<Cell> &pathToExit, GridPosition &pos, unsigned int index)
{
	...
	// set the lock to access the segment list of the current lock
...
void travelerExit(unsigned int index)
{
	...
	while (!travelerList[index].segmentList.empty())
	{
		...
		// each segment of the traveler disappearing into the exit point
		// counts as a change in position that needs to be drawn
		travelerLocks[index].lock();
		...
		// mark the head as an EXIT square since it is at the exit point
		unsigned int headRow = travelerList[index].segmentList[0].row;
		unsigned int headCol = travelerList[index].segmentList[0].col;

		globalTravelerLock.lock();

		grid[headRow][headCol] = SquareType::EXIT;

		// mark the tail as a FREE_SQUARE unless it is the last remaining segment
		unsigned int tailRow = travelerList[index].segmentList[travelerList[index].segmentList.size() - 1].row;
		unsigned int tailCol = travelerList[index].segmentList[travelerList[index].segmentList.size() - 1].col;

		if (travelerList[index].segmentList.size() > 1)
		{
			grid[tailRow][tailCol] = SquareType::FREE_SQUARE;
		}
		else
		{
			grid[tailRow][tailCol] = SquareType::EXIT;
		}
